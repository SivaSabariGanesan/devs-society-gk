...
const fixAdminCollegeMapping = async () => {
  try {
    ...
    const admin = await AdminService.findByEmail('admin.nitk@devs-society.com')
    if (!admin) {
    ...
    // Verify the fix
    console.log('\n🔍 Verifying fix...')
    const updatedAdmin = await AdminService.findByEmail('admin.nitk@devs-society.com')
    if (updatedAdmin) {
      ...
      if (updatedAdmin.assignedCollege) {
        const assignedCollege = await CollegeService.findById(
          typeof updatedAdmin.assignedCollege === 'object' ? 
