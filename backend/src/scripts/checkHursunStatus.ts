...
const checkHursunStatus = async () => {
  try {
    ...
    const adminByEmail = await AdminService.findByEmail('hursun@gmail.com')
    if (adminByEmail) {
    ...
    if (error) {
      ...
      if (rawAdmin.college_tenure_heads && rawAdmin.college_tenure_heads.length > 0) {
        rawAdmin.college_tenure_heads.forEach((tenure: any, index: number) => {
    ...
    try {
      ...
      if (apiError) {
        ...
        const hursunFromAPI = apiAdmins?.find((admin: any) => admin.email === 'hursun@gmail.com')
        if (hursunFromAPI) {
          ...
          if (hursunFromAPI.college_tenure_heads && hursunFromAPI.college_tenure_heads.length > 0) {
            const activeTenure = hursunFromAPI.college_tenure_heads.find((t: any) => t.is_active)
            if (activeTenure) {
